...
async function downloadResultCard(event) {
    ...
    if (btn) {
        ...
        icon.textContent = '⏳ ';
        btn.appendChild(icon);
        btn.appendChild(document.createTextNode('이미지 생성 중...'));
    }

    // Yield execution to the main thread so the browser can render the loading state
    ...
    try {
            ...
            try {
                ...
                if (blob) {
                    ...
                    if (navigator.canShare({ files: [file] })) {
                        ...
                        return; // Share success, but we still go to finally block
                    }
                }
            } catch (err) {
                if (err.name === 'AbortError') {
                    shareAborted = true;
                    return; // Return early, finally block handles restore without success message
                }
        ...
        if (btn) {
            ...
            icon.textContent = '✅ ';
            btn.appendChild(icon);
            btn.appendChild(document.createTextNode('저장 완료!'));
        }
    } finally {
        if (btn) {
            setTimeout(() => {
