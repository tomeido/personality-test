...
## 2024-09-12 - Inline Feedback for Synchronous Operations
**Learning:** Providing UI feedback (like loading or success states) on buttons during heavy synchronous operations (like building a large canvas or triggering downloads) requires explicitly yielding execution back to the browser using `await new Promise(resolve => setTimeout(resolve, 50))`. Without this yield, the UI won't have time to update the button's text before the main thread freezes for the heavy operation. Also, using `cloneNode(true)` is essential to capture complex button children (like spans with icons and text) to properly restore them after showing feedback.
**Action:** When adding loading/success text to buttons triggering canvas generation or native downloads, clone original children, update the text, yield with a tiny timeout, execute the work, and then restore the children after a success timeout.
