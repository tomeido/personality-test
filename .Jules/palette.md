...
## 2025-02-14 - Add Async Loading State for Canvas Generation
**Learning:** Heavy synchronous DOM operations (like `canvas.toDataURL()` or building complex canvas graphics) block the main thread, preventing the browser from rendering inline loading states if triggered in the same frame. Additionally, dynamically inserted emojis need `aria-hidden="true"` to prevent redundant screen reader announcements.
**Action:** Always yield execution to the main thread via `await new Promise(resolve => setTimeout(resolve, 50))` after setting a loading state (e.g., `btn.innerHTML = '<span aria-hidden="true">⏳</span> 이미지 생성 중...'`) to guarantee the UI is painted before synchronous blocking code executes.
