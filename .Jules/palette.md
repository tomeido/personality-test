...
## 2024-05-18 - Safe Async UI Loading States
**Learning:** When displaying inline loading states (e.g. `innerHTML = '⏳...'`) right before heavy synchronous operations like canvas rendering, the browser main thread will block immediately. If execution is not explicitly yielded, the updated DOM will never render to the screen. Additionally, when backing up and restoring button contents, using `cloneNode(true)` is critical; simply mapping childNodes without cloning can lead to complex elements (like SVGs or emojis) being lost or mangled when re-appended.
**Action:** Always insert a micro-task delay (e.g. `await new Promise(r => setTimeout(r, 50))`) between setting a loading state and running synchronous blockers to ensure the browser paints the update. Use `cloneNode(true)` for safely preserving and restoring DOM state inline.
