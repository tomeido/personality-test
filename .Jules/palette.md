...
## 2024-11-21 - Feedback Before Heavy Synchronous Operations
**Learning:** For heavy synchronous operations like canvas rendering (`buildResultCardCanvas()`), manipulating the DOM immediately beforehand won't visually update because the main thread gets blocked instantly. Users experience a "frozen" UI rather than a loading state.
**Action:** When providing inline UI feedback (e.g., loading states) right before heavy synchronous operations, explicitly yield execution to the main thread (e.g., `await new Promise(resolve => setTimeout(resolve, 50))`) to allow the browser to paint the updated DOM before it blocks.
